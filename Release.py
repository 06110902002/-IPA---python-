...
def createFolder(targetName):	
	d = time.ctime().split(' ')
	curTime = d[3]
	dirStr = '%s/%s' %(TARGET_FOLDERPATH,targetName)
...
def uploadApk():
    apkServerUrl = 'http://localhost:8080/PkgTest/'
    apkPath = TARGET_FOLDERPATH + '/' + TARGET_NAME + '/' + TARGET_NAME +'.ipa'
    #print'86-------------------:',TARGET_FOLDERPATH + '/' + TARGET_NAME + '/' + TARGET_NAME +'.ipa'

    try:
        file = {'file': open(apkPath, 'rb')}
        req = requests.post(url= apkServerUrl, files=file)
        print '89---------------:upload result:',req.text
    except Exception as e:
        print'91---------------upload apk error:',e
